...
const TransferFee = (props: Props) => {
    const utils = api.useUtils();
    const {data: fee, isLoading: isLoadingFee} = api.fee.getFee.useQuery({type: "Transfer"})
    const {mutateAsync: mutateFee, isLoading} = api.fee.updateFee.useMutation(
        {
            onSuccess: (res) => {
                if(res) {
                    utils.fee.getFee.setData({type: "Transfer"}, (prev) => {
                        return res.fee
                    })
    ...
    function onSubmit(data: z.infer<typeof FormSchema>) {
        mutateFee({newFee: parseFloat(data.fee), type: "Transfer"})
    }
