...
const BonusReferral = (props: Props) => 
{

    const utils = api.useUtils();
    const {data: fee, isLoading: isLoadingFee} = api.fee.getFee.useQuery({type: "Bonus"})
    const {mutateAsync: mutateFee, isLoading} = api.fee.updateFee.useMutation(
        {
            onSuccess: (res) => 
            {
                if(res)
                {
                    utils.fee.getFee.setData({type: "Bonus"}, (prev) => 
                    {
                        return res.fee
                    })
    ...
    {
        mutateFee({newFee: parseFloat(data.fee.replace(',', '.')), type: "Bonus"})
    }
    ...
    return (
        <div className="flex h-[20rem]">
            <div className="flex-1 p-4 bg-gray-200 flex w-full items-center justify-center">
                <div>
                    <p className="font-bold">Поточний бонус на депозит:</p>
                ...
                <div>
                        <Form {...form}>
                            ...
                            <FormInput control={control} name="fee" label="Нова поточна ставка:" placeholder="1%" transformChangeValue={(value, prevValue)=>
                                ...
                                <Button type="submit" className="w-[16rem]">
                                {
                                    isLoading ? 
                                    <SpinnerLoading color="#000000"/> 
                                    :
                                    "Встановити відсоток бонусу"
                                }
                                </Button>
                                </div>
                            </form>
