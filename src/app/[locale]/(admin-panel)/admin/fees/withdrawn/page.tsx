...
const WithdrawnFee = (props: Props) => {
    const utils = api.useUtils();
    const {data: fee, isLoading: isLoadingFee} = api.fee.getFee.useQuery({type: "Widthdraw"})
    const {mutateAsync: mutateFee, isLoading} = api.fee.updateFee.useMutation(
        {
            onSuccess: (res) => {
                if(res) {
                    utils.fee.getFee.setData({type: "Widthdraw"}, (prev) => {
                        return res.fee
                    })
    ...
    function onSubmit(data: z.infer<typeof FormSchema>) {
        mutateFee({newFee: parseFloat(data.fee), type: "Widthdraw"})
    }
