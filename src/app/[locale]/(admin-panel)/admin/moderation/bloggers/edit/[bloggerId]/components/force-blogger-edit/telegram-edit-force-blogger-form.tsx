...
const schema = z.object({
    ...
    prices: z.record(z.enum(['post-30x24', 'post-1x24', 'post-2x48', 'post-3x72', 'no-deletion']), z.object({
        price: z.string(),
    ...
    channelAge: z.string().regex(/^\d*\.?\d*$/, 'Введіть коректне число').refine(val => !val || (parseFloat(val) >= 0 && parseFloat(val) < 100), 'Введіть число від 0 до 100').optional(),
}).refine(data => {
...
const TelegramForceBloggerEditForm = ({ blogger, isLoading = false }: Props) => {
...
export default TelegramForceBloggerEditForm