...
const ActionsCell = ({ row }: CellContext<TableData, unknown>) => {
    ...
    return <ActionsDropdown open={open} setOpen={setOpen}>
        <ActionItem>
            <Link href={`/admin/moderation/bloggers/${row.original.id}`} className="flex items-center gap-x-2">
                <Eye size={18} />
                <Translate namespace="Blogger" itemKey="watch" />
            </Link>
        </ActionItem>
        <ActionItem>
            <Link href={`/admin/moderation/bloggers/edit/${row.original.id}`} className="flex items-center gap-x-2">
                <EditIcon size={18} />
                Редагувати
            </Link>
        </ActionItem>
        <ActionItem>
            <div
                ...
                onClick={
                    () => {
                        removeBlogger.mutate({ bloggerId: row.original.id })
                        utils.admin.blogger.getModerationBloggers.setData(undefined, (prevData) => {
                            return prevData?.filter(blogger => blogger.id != row.original.id)
                        })
                        setOpen(false)
                    }
                }
