...
const TransferMoneyModalBlogger = ({ children }: Props) => {
    ...
    const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (/^\d*\.?\d*$/.test(value)) {
            ...
            setNewBloggerBalance((user?.bloggerBalance ?? 0) - numAmount);
            setNewAdvertiserBalance((user?.advertiserBalance ?? 0) + amountAfterFee);
        } else {
    ...
    const { mutate: transferMoney, error, isLoading } = api.blogger.transactions.transferMoney.useMutation({
        onSuccess: () => {
            setOpen(false);
            refetch();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (numAmount > (user?.bloggerBalance ?? 0)) {
                ...
                <div className="flex flex-col sm:flex-row justify-between mb-6">
                    ...
                    <div className="space-y-2 text-right">
                        <div>
                            ...
                            <span className="text-lg text-rose-500 font-bold ml-2">{newBloggerBalance.toFixed(2)} ₴</span>
                        </div>
                        <div>
                            <span className="text-md font-medium">
                                <Translate namespace='Default' itemKey='advertiserBalance' />:
                            </span>
                            <span className="text-lg text-green-500 font-bold ml-2">{newAdvertiserBalance.toFixed(2)} ₴</span>
                        </div>
                    </div>
                </div>
...
export default TransferMoneyModalBlogger