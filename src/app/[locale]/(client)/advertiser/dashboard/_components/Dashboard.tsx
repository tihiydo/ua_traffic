...
import { columns as postColumns } from '../../my-posts/_components/columns'
...
const SectionHeader: React.FC<SectionHeaderProps> = ({ icon: Icon, className, locale, title, link, onAdd, showChevron = true }) => (
...
const Dashboard: React.FC = () => {
    ...
    const { data: advertismentRequests, isLoading: isRequestsLoading } = api.advertisment.requests.getMyAdvertiserRequests.useQuery()
    const { data: advertisments, isLoading: isPostsLoading } = api.advertisment.posts.getMyPosts.useQuery()
    const { data: chats = [], isLoading: isChatsLoading } = api.chat.getAdvertiserChats.useQuery(undefined, {
    ...
    const handleChatSelect = (chat: RealtimeChat) => {
        router.push(`/${locale}/advertiser/chats?chat=${chat.chat.id}`);
    };
    ...
    return (
        <>
            <h1 className="text-[30px] leading-[35px] font-bold mb-4">
                <Translate namespace="Advertiser" itemKey="dashboard" />
            </h1>
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 max-w-full">
                <div className="space-y-8 xl:col-span-2">
                    <section className="border border-gray-300 rounded-lg">
                        <SectionHeader 
                            icon={FileText} 
                            title={<Translate namespace="Advertiser" itemKey="mysposts" />}
                        ...
                        <DataTable
                            ...
                            data={(advertismentRequests || []).slice(0, 4)}
                    ...
                    <section className="border border-gray-300 rounded-lg">
                        <SectionHeader 
                            icon={Folders} 
                            title={<Translate namespace="Advertiser" itemKey="orders" />}
                ...
                <div className="xl:col-span-1">
                    <section className="border border-gray-300 rounded-lg flex flex-col h-full">
                        <SectionHeader 
                            icon={MessageCircle} 
                            title={<Translate namespace="Advertiser" itemKey="chats" />}
                ...
                <div className="xl:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <section className="border border-gray-300 rounded-lg md:col-span-2">
                        <SectionHeader 
                            icon={Wallet} 
                            title={<Translate namespace="Advertiser" itemKey="billing" />}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-5 p-4 gap-4">
                            <div className="bg-white p-4 border border-gray-300 rounded-lg sm:col-span-3">
                                ...
                                <p className='font-bold text-[20px] leading-[24px]'>{user?.advertiserBalance} ₴</p>
                                <div className='flex flex-col sm:flex-row gap-2 mt-4'>
                                    <Button variant="default" className="w-full">
                                        <Link href={`/${locale}/advertiser/billing`}>
                                            <p className="font-bold text-[18px] leading-[21px]"><Translate namespace="Advertiser" itemKey="addmoney" /></p>
                                        </Link>
                                    </Button>
                                    <TransferMoneyModal>
                        <Button variant='outline' className='w-full'>
                            ...
                            <div className="bg-white p-4 border border-gray-300 rounded-lg sm:col-span-2">
                                ...
                                <p className="font-bold text-[20px] leading-[24px]">{user?.advertiserHoldBalance} ₴</p>
                            </div>
                        </div>
                    </section>
                </div>
