...
const InstagramFilters = ({ settings }: Props) => {
    const { clear, update, merge, schemaParams, include } = useCatalogParams(settings);
    ...
    return (
        <div>
            ...
            <div className='mt-5'>
                ...
                <PostTypesSelect
                    postTypes={Object.values(InstagramAdPostType)}
                    value={Object.values(InstagramAdPostType).includes(filters.postType as InstagramAdPostType) ? filters.postType as InstagramAdPostType : undefined}
                    onChange={(value) => {
                        const realValue = value || "all"
                        if (realValue == "all") {
                            merge({
            ...
            <FilterSlider
                onChange={(value) => {
                    const min = filters.minSubs ?? settings.minSubs;
                    const max = filters.maxSubs ?? settings.maxSubs;
                    if (min === value[0] && max === value[1]) return;
